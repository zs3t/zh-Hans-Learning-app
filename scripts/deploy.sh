...
DEST_BASE_DIR="$HOME" # 默认为当前用户的主目录，通常是 /home/youruser 或 /root
...
echo "正在从 Git 拉取最新代码..."
git fetch # 先获取最新变更
# 假定你的主分支是 main。如果你的主分支是 master，请修改为 origin/master
git reset --hard origin/main || { echo "❌ 错误: Git reset --hard 失败。"; exit 1; }
echo "✅ 代码已更新到最新版本。"
...
echo "📦 安装 npm 依赖 (包括开发依赖以支持构建)..."
# **重要修改：将 --production 移除，确保安装所有必要的构建依赖，如 autoprefixer**
npm install || { echo "❌ 错误: npm install 失败。请检查网络连接或 package.json。"; exit 1; }
...
npm run build || { echo "❌ 错误: Next.js 构建失败。请检查编译错误。"; exit 1; }
echo "✅ Next.js 应用构建成功。"
